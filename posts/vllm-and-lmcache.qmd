...
vLLM solves the memory management problem when serving LLMs. The big innovation is PagedAttention - think of it like this: instead of requiring continuous memory blocks (like needing an entire empty shelf for a book), it lets you split things across wherever there's space. This sounds simple but it's huge - I typically see 2-3x better throughput just from this.
...
LMCache lets you cache KV pairs across requests. This matters because most of my applications have repeated prefixes:
...
The latency improvement is noticeable - first request is normal, subsequent requests are significantly faster because you skip computing those 900 tokens of instructions and examples.
...
- Mistral 7B v0.3: Similar to Llama 3.1 8B, 30-35 users
- Mixtral 8x7B: ~10-12 concurrent users
...
The jump from Llama 2 to Llama 3.1 was significant - not just better quality but also better memory efficiency.
...
5. **GQA is a game changer**
   ...
   - Always prefer models with GQA when possible
...
- Plan 1-1.5 GB per concurrent user for Llama 3.1 8B
- A100 80GB can handle ~35-40 users for 8B models with 8K context
- Use LMCache for repeated prefixes
- Always leave 5-10 GB buffer

**Why Llama 3.1 over Llama 2:**
- Better quality
- 4x less KV cache memory (GQA with 8 heads vs 32)
- Supports up to 128K context (though I usually stick to 8K)
- Better instruction following

vLLM and LMCache turned LLM deployment from painful to manageable. The performance difference is too significant to ignore.
